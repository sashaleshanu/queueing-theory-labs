...
def task_2():
    ...
    #время обслуживания заявки прибором
    t_service = 0.959 
    ...
    times_of_coming = sps.expon.rvs(scale=1/l, size=N)

    #наступление первого события
    t_event = times_of_coming[0]
    current_event_stat = 1
    current_sys_stat = 1

    #сколько времени выполняется текущая заявка
    remaining_sevr_time = t_service

    #когда заяка "попросится" на выполнение
    remaining_demand_time = times_of_coming[1]
    ...
    stays_total = times_of_coming[0]

    table_1 = []
    table_2 = [None] * 101

     #добавляем в таблицу 2 первый ивент:
        #номер заявки
    ...
    for i in range(2, 101):
        ...
        if remaining_sevr_time < remaining_demand_time and current_sys_stat == 1:
            ...
            remaining_demand_time = times_of_coming[j]
            t_j = i - t_total
            t_rejected_total += 1
